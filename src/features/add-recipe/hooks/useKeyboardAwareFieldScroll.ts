...
export function useKeyboardAwareFieldScroll<T = ScrollView>(
  ...
  const scrollFocusedIntoView = (kbHeight?: number) => {
    ...
    const responder =
      ...
      scrollView.getScrollResponder?.()?.scrollResponderScrollNativeHandleToKeyboard;
  ...
  const ensureFocusedVisible = (kbHeight?: number) => {
